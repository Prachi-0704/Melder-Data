# Use Case 012: Number Normalization

## Scenario
Numeric or ordinal spellings in address or company names should resolve to the same record.
...
```bash
./usecases/run_usecase.sh uc_012_number_normalization
```
...
```bash
./target/release/meld enroll --config usecases/uc_012_number_normalization/config_enroll.yaml --port 8090
```

Keep the enroll output separate from batch `run_###` folders by using a dedicated run directory such as `usecases/uc_012_number_normalization/runs/enroll_run_001/` before starting the server.
Run (from repo root):

```bash
cd melder && ./usecases/run_usecase.sh uc_012_number_normalization
```
