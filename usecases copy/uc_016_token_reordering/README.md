# Use Case 016: Token Reordering

## Scenario
The order of tokens in a company name should not determine whether records are the same entity.
...
```bash
./usecases/run_usecase.sh uc_016_token_reordering
```
...
```bash
./target/release/meld enroll --config usecases/uc_016_token_reordering/config_enroll.yaml --port 8090
```

Keep the enroll output separate from batch `run_###` folders by using a dedicated run directory such as `usecases/uc_016_token_reordering/runs/enroll_run_001/` before starting the server.
Run (from repo root):

```bash
cd melder && ./usecases/run_usecase.sh uc_016_token_reordering
```
