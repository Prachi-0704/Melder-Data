# Use Case 003: Special Character Handling

## Scenario
Punctuation and formatting differences in name and address fields should still resolve to the same entity.
...
```bash
./usecases/run_usecase.sh uc_003_special_character_handling
```
...
```bash
./target/release/meld enroll --config usecases/uc_003_special_character_handling/config_enroll.yaml --port 8090
```

Keep the enroll output separate from batch `run_###` folders by using a dedicated run directory such as `usecases/uc_003_special_character_handling/runs/enroll_run_001/` before starting the server.
Run (from repo root):

```bash
cd melder && ./usecases/run_usecase.sh uc_003_special_character_handling
```
