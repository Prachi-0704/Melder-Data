# Use Case 017: Contains / Partial Match

## Scenario
Records with partial word overlap and minor spelling drift often identify the same legal entity.
...
```bash
./usecases/run_usecase.sh uc_017_contains_partial_match
```
...
```bash
./target/release/meld enroll --config usecases/uc_017_contains_partial_match/config_enroll.yaml --port 8090
```

Keep the enroll output separate from batch `run_###` folders by using a dedicated run directory such as `usecases/uc_017_contains_partial_match/runs/enroll_run_001/` before starting the server.
Run (from repo root):

```bash
cd melder && ./usecases/run_usecase.sh uc_017_contains_partial_match
```
