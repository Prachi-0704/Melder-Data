...
/// - For `numeric_magnitude` fields: additive bonus rewarding magnitude of
///   whichever side parses higher, scaled by `scale_max`
/// - For `preferred_value` fields: additive bonus rewarding a match against
///   a configured list of trusted values, case-insensitive, on either side
///
...
pub fn score_pair(
    ...
    let mut base_weighted_sum = 0.0_f64; // non-additive methods
    let mut additive_bonus = 0.0_f64; // additive bonus: synonym, numeric_magnitude, preferred_value
    let mut total_weight = 0.0_f64; // denominator (excludes additive methods)
    ...
    for mf in match_fields {
        let score = if mf.method == MatchMethod::Bm25 {
            ...
            match mf.method {
                ...
                MatchMethod::NumericMagnitude => {
                    numeric_magnitude_score(a_val, b_val, mf.scale_max.unwrap_or(0.0))
                }
                MatchMethod::PreferredValue => preferred_value_score(
                    a_val,
                    b_val,
                    mf.preferred_values.as_deref().unwrap_or(&[]),
                ),
        ...
        // Synonym, numeric_magnitude, and preferred_value are flat additive
        // bonuses: score * weight is added directly on top of the base
        // composite. Their contribution is never diluted by weight
        // normalization — only the base (non-additive) methods normalize
        // among themselves.
        let is_additive = matches!(
            mf.method,
            MatchMethod::Synonym | MatchMethod::NumericMagnitude | MatchMethod::PreferredValue
        );
        if is_additive {
            additive_bonus += score * mf.weight;
        } else {
    ...
    // Normalize the base score if non-additive weights don't sum to 1.0,
    // then add the additive bonus (synonym + numeric_magnitude +
    // preferred_value) on top, and clamp to [0, 1].
    let base = if total_weight > 0.0 && (total_weight - 1.0).abs() > 0.001 {
        base_weighted_sum / total_weight
    } else {
        base_weighted_sum
    };
    let total = if (base + additive_bonus).is_nan() {
        0.0
    } else {
        base + additive_bonus
    }
...
/// Numeric magnitude scorer: rewards magnitude, ignoring agreement between
/// `a` and `b`. Uses whichever side parses higher, so a strong value on
/// either the query or candidate record drives the boost.
///
/// Returns 0.0 if `scale_max <= 0.0` or if neither side parses as a number.
fn numeric_magnitude_score(a: &str, b: &str, scale_max: f64) -> f64 {
    if scale_max <= 0.0 {
        return 0.0;
    }
...
/// Preferred-value scorer: 1.0 if either side's value matches (case-insensitive)
/// one of the configured preferred values, else 0.0.
fn preferred_value_score(a: &str, b: &str, preferred: &[String]) -> f64 {
    let a = a.trim();
    let b = b.trim();

    if preferred
        .iter()
        .any(|v| v.eq_ignore_ascii_case(a) || v.eq_ignore_ascii_case(b))
    {
        1.0
    } else {
        0.0
...
mod tests {
    ...
    #[test]
    fn numeric_magnitude_rewards_higher_value_regardless_of_agreement() {
        // max(20, 3) / 10.0 -> clamped to 1.0
        assert_eq!(numeric_magnitude_score("20", "3", 10.0), 1.0);
    }

    #[test]
    fn numeric_magnitude_scales_below_max() {
        // max(5, 0) / 10.0 = 0.5
        assert_eq!(numeric_magnitude_score("5", "0", 10.0), 0.5);
    }

    #[test]
    fn numeric_magnitude_zero_scale_max_is_zero() {
        assert_eq!(numeric_magnitude_score("100", "100", 0.0), 0.0);
    }

    #[test]
    fn numeric_magnitude_unparseable_is_zero() {
        assert_eq!(numeric_magnitude_score("abc", "def", 10.0), 0.0);
    }

    #[test]
    fn preferred_value_matches_case_insensitive() {
        let preferred = vec!["SAP".to_string()];
        assert_eq!(preferred_value_score("sap", "other", &preferred), 1.0);
    }

    #[test]
    fn preferred_value_matches_either_side() {
        let preferred = vec!["Salesforce".to_string()];
        assert_eq!(preferred_value_score("other", "Salesforce", &preferred), 1.0);
    }

    #[test]
    fn preferred_value_no_match_is_zero() {
        let preferred = vec!["SAP".to_string(), "NetSuite".to_string()];
        assert_eq!(preferred_value_score("Oracle", "Workday", &preferred), 0.0);
    }

    #[test]
    fn numeric_magnitude_and_preferred_value_add_on_top_without_diluting_base() {
        // Both sides carry identical values for every field, so an exact-match
        // field_a/field_b naming works symmetrically regardless of which side
        // is passed as query vs. candidate.
        let a = make_record(&[
            ("name", "Acme Corp"),
            ("linkages", "25"),
            ("system", "SAP"),
        ...
        let fields = vec![
            ...
            MatchField {
                field_a: "linkages".into(),
                field_b: "linkages".into(),
                method: MatchMethod::NumericMagnitude,
                scorer: None,
                weight: 0.10,
                fields: None,
                scale_max: Some(20.0),
            ...
            MatchField {
                field_a: "system".into(),
                field_b: "system".into(),
                method: MatchMethod::PreferredValue,
                scorer: None,
                weight: 0.05,
                fields: None,
                scale_max: None,
                preferred_values: Some(vec!["SAP".to_string()]),
            },
        ];

        let result = score_pair(&b, &a, &fields, None, None, None, Side::A);

        // base: exact name match, weight 1.0, total_weight 1.0 -> base = 1.0
        // additive: numeric_magnitude 25/20 clamped to 1.0 * 0.10 = 0.10
        //           preferred_value 1.0 * 0.05 = 0.05
        // raw total = 1.15, clamped to 1.0
