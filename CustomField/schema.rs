...
pub enum MatchMethod {
    ...
    Bm25,
    Synonym,
    /// Additive bonus: rewards magnitude of a numeric field (e.g. count of
    /// linkages), independent of whether the two sides agree. See
    /// `MatchField::scale_max`.
    NumericMagnitude,
    /// Additive bonus: rewards records whose value on either side matches a
    /// configured list of trusted/preferred values. See
    /// `MatchField::preferred_values`.
    PreferredValue,
}
...
impl MatchMethod {
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            MatchMethod::NumericMagnitude => "numeric_magnitude",
            MatchMethod::PreferredValue => "preferred_value",
        }
    }
}
...
pub struct MatchField {
    ...
    /// Denominator for `method: numeric_magnitude` scoring. A value >=
    /// `scale_max` scores 1.0; values below scale linearly toward 0.0.
    /// Required (and must be > 0) when `method == MatchMethod::NumericMagnitude`;
    /// ignored otherwise.
    #[serde(default)]
    pub scale_max: Option<f64>,
    /// Trusted/preferred values for `method: preferred_value` scoring. If
    /// either side's field value case-insensitively matches any entry, the
    /// field scores 1.0, else 0.0. Required (and must contain at least one
    /// non-empty value) when `method == MatchMethod::PreferredValue`;
    /// ignored otherwise.
    #[serde(default)]
    pub preferred_values: Option<Vec<String>>,
}
