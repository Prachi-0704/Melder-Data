...
fn validate_match_fields(cfg: &Config) -> Result<(), ConfigError> {
    ...
    for (i, mf) in cfg.match_fields.iter().enumerate() {
        ...
        // Additive bonus methods (synonym, numeric_magnitude, preferred_value)
        // reward record quality rather than comparing the two sides, so they
        // are excluded from the weight-sum-to-1.0 pool. Their weights are
        // applied as a flat bonus on top of the base composite score in
        // `scoring::score_pair`.
        ...
        if mf.method == MatchMethod::NumericMagnitude {
            let valid = mf.scale_max.is_some_and(|v| v > 0.0);
            if !valid {
                return Err(ConfigError::InvalidValue {
                    field: format!("{}.scale_max", prefix),
                    message: "required and must be > 0 for method: numeric_magnitude".into(),
                });
            }
        }

        if mf.method == MatchMethod::PreferredValue {
            let valid = mf.preferred_values.as_ref().is_some_and(|vals| {
                !vals.is_empty() && vals.iter().all(|v| !v.trim().is_empty())
            });
            if !valid {
                return Err(ConfigError::InvalidValue {
                    field: format!("{}.preferred_values", prefix),
                    message:
                        "required and must contain at least one non-empty value for method: preferred_value"
                            .into(),
                });
            }
        }
...
"#,
    ...
    // --- Additive bonus methods: numeric_magnitude / preferred_value ---

    #[test]
    fn numeric_magnitude_requires_scale_max() {
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: linkages, field_b: linkages, method: numeric_magnitude, weight: 0.10 }",
        );
        let err = parse_and_validate(&yaml).unwrap_err();
        assert!(
            err.to_string().contains("numeric_magnitude"),
            "got: {}",
            err
        );
    }
    ...
    #[test]
    fn numeric_magnitude_scale_max_zero_rejected() {
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: linkages, field_b: linkages, method: numeric_magnitude, weight: 0.10, scale_max: 0.0 }",
        );
        let err = parse_and_validate(&yaml).unwrap_err();
        assert!(
            err.to_string().contains("numeric_magnitude"),
    ...
    #[test]
    fn numeric_magnitude_valid_scale_max_accepted() {
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: linkages, field_b: linkages, method: numeric_magnitude, weight: 0.10, scale_max: 20.0 }",
        );
        parse_and_validate(&yaml).unwrap();
    }

    #[test]
    fn numeric_magnitude_weight_excluded_from_weight_sum() {
        // base fields already sum to 1.0; numeric_magnitude weight (0.10)
        // must NOT push weight_sum over the tolerance.
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: linkages, field_b: linkages, method: numeric_magnitude, weight: 0.10, scale_max: 20.0 }",
        );
        parse_and_validate(&yaml).unwrap();
    ...
    #[test]
    fn preferred_value_requires_preferred_values() {
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: system, field_b: system, method: preferred_value, weight: 0.05 }",
        );
        let err = parse_and_validate(&yaml).unwrap_err();
        assert!(
            err.to_string().contains("preferred_value"),
            "got: {}",
            err
        );
    }
    ...
    #[test]
    fn preferred_value_empty_list_rejected() {
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: system, field_b: system, method: preferred_value, weight: 0.05, preferred_values: [] }",
        );
        let err = parse_and_validate(&yaml).unwrap_err();
        assert!(
            err.to_string().contains("preferred_value"),
    ...
    #[test]
    fn preferred_value_blank_entry_rejected() {
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: system, field_b: system, method: preferred_value, weight: 0.05, preferred_values: [\"   \"] }",
        );
        let err = parse_and_validate(&yaml).unwrap_err();
    ...
    #[test]
    fn preferred_value_valid_list_accepted() {
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: system, field_b: system, method: preferred_value, weight: 0.05, preferred_values: [\"SAP\", \"Salesforce\", \"NetSuite\"] }",
        );
        parse_and_validate(&yaml).unwrap();
    }

    #[test]
    fn numeric_magnitude_and_preferred_value_together_accepted() {
        let yaml = base_yaml_with_match_fields(
            "  - { field_a: f, field_b: f, method: exact, weight: 1.0 }\n  - { field_a: linkages, field_b: linkages, method: numeric_magnitude, weight: 0.10, scale_max: 20.0 }\n  - { field_a: system, field_b: system, method: preferred_value, weight: 0.05, preferred_values: [\"SAP\"] }",
        );
        parse_and_validate(&yaml).unwrap();
    }
}