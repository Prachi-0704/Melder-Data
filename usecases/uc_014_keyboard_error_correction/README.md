# Use Case 014: Keyboard Error Correction

## Scenario
Typographical errors in company or address names should still resolve to the same entity.
...
```bash
./usecases/run_usecase.sh uc_014_keyboard_error_correction
```
...
```bash
./target/release/meld enroll --config usecases/uc_014_keyboard_error_correction/config_enroll.yaml --port 8090
```

Keep the enroll output separate from batch `run_###` folders by using a dedicated run directory such as `usecases/uc_014_keyboard_error_correction/runs/enroll_run_001/` before starting the server.
Run (from repo root):

```bash
cd melder && ./usecases/run_usecase.sh uc_014_keyboard_error_correction
```
